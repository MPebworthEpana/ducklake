...
public:
	...
	//! Persist a CREATE TYPE as a schema tag (key=udt:<name>) and, when supported, a formal ducklake_type row.
	void RegisterUserType(SchemaIndex schema_id, const string &type_name, const LogicalType &type);
	//! End-snapshot a previously persisted user type tag (and formal row) on DROP TYPE.
