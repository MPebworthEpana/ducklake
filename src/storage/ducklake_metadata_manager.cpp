...
WHERE snapshot_id = %llu;)",
	...
	if (transaction.HasActiveBranch()) {
...
WHERE snapshot_id = (
	...
	LIMIT 1);)",
		...
		// Phase 1: resolve named ref → snapshot_id. Refs are immutable in P1 except
		// drop/recreate; the transaction-scoped snapshot_cache keyed by unit+value is safe.
